...
const pool = new Pool({
 ...
 password: '20PostGres25', // anpassen
 port: 5432,
});
...
app.post('/api/kunden', async (req, res) => {
 const { nachname, vorname, email } = req.body; // Daten aus dem Request-Body mit Destructuring extrahieren. Es werden drei Variablen erstellt: nachname, vorname und email
 try {
 await pool.query(
 'INSERT INTO kunden (nachname, vorname, email) VALUES ($1, $2, $3)',
 [nachname, vorname, email] // Daten aus dem Request-Body in die Datenbank einfügen
 );
